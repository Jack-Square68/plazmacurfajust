...
Closed openings measure width on a capped polyline (so Rhino stays live). The baked outline keeps the **original NURBS** (few edit points): each wall is offset as a span, then every tip is closed with the **same Min width / 2** G1 arc. Slot ends get a **Min width / 2** semicircle around the original tip. Pointed corners of a wide opening stay as drawn. The result is never rebuilt as a dense interpolant. That is what stopped the old script from locking Rhino on a koru: it used to fire `GetLength` / `Contains` on the NURBS for every sample.
